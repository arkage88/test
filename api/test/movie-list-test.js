...
const testCase = {
 "positive" : {
    ...
    "getDetail" : "As a User, I want to be able to get Detail OMDB Movie list",
 },
 "negative" : {
...
describe(`OMDB Movie List`, () => {
 ...
 const keySearch = 'fast';
 const keyTitle = 'Fast Five';
 ...
 it(`@get ${testCase.positive.getList}`, async() => {
  ...
  it(`@get ${testCase.positive.getDetail}`, async() => {
   const response = await page.getMovieList(apiKey, '', keyTitle);
   assert(response.status).to.equal(200);
   console.log(response.body);
  }),
